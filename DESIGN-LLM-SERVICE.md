...
### 9-1. 뼈대/상품 2단계 병렬 생성 (2026-08 도입)

계획 생성은 두 LLM 호출의 병렬이다: ① **뼈대**(`PLAN_SKELETON_SYSTEM`, 검색 없음·effort
medium) — headline·summary·guide·steps와 상품 섹션 "자리"(제목·기준 reason만, 구체 상품명
금지)를 수 초 안에 스트리밍. ② **상품**(`PLAN_PRODUCTS_SYSTEM`, 웹 검색 포함·high) —
products 섹션 1~2개만 생성, 그라운딩 후 자리를 채운다.

- **병합·인덱스 규칙은 `plan-merge.ts`** (스트리밍과 최종 병합이 같은 규칙): 뼈대 비상품
  섹션은 뼈대 인덱스 그대로, k번째 상품 섹션 = k번째 자리 인덱스(자리 초과분은 뼈대 길이
  뒤). 자리 인덱스는 뼈대 **최종 검증본** 기준으로 확정하고, 먼저 도착한 상품 섹션은
  뼈대 완료까지 대기열에 쌓았다 내보낸다 (실측상 뼈대가 항상 먼저 끝난다)
- SSE 계약 변경 없음 — FE는 희소 인덱스 삽입(`section.index`가 자리 위치)으로 그대로 소화
- 상품 단계 실패는 계획을 죽이지 않는다 — 상품 없는 계획을 반환하고 로그만 (뼈대 실패는 error)
