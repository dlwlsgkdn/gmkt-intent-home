...
### 9-1. 검토됨 — 뼈대/상품 2단계 병렬 생성 (보류)

계획 생성을 두 호출로 분리하는 안: ① 검색 없는 빠른 호출이 headline·summary·guide·steps
뼈대를 수 초 안에 스트리밍 → ② 병렬 검색 호출이 products 섹션을 완성하면 슬롯 교체.
첫 콘텐츠를 확실히 수 초 안에 보장하지만 다음 비용이 있어 **보류**한다:

- LLM 호출 2회 — 토큰 비용과 시스템 프롬프트 캐시 이중화
- 뼈대와 상품 문구의 톤·논리 정합을 별도로 관리해야 함 (뼈대가 상품을 모른 채 안내를 씀)
- SSE 계약에 "섹션 교체/삽입" 이벤트가 추가로 필요 (지금은 append-only)
- 저장 스텝 병합 시점이 둘로 갈라짐 (멱등 seq 구조 재설계)

**도입 기준**: 현행 최적화(클라이언트 선렌더 + 프롬프트 작성 순서 유도 v4)의 실측에서
"첫 LLM 콘텐츠 도착"이 여전히 10초를 크게 넘으면 그때 재검토. 순서 유도가 먹히면
검색 전 텍스트가 먼저 흐르므로 2단계 분리의 이득 대부분이 이미 확보된다.
