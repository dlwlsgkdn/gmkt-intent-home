...
> pause_turn 연속 호출에도 같은 파서 인스턴스가 이어받는다. 상품·콘텐츠 섹션은 원소 완성
> 전에도 **항목 단위 증분**으로 나간다(`completeSearchSection` — 복구 파싱 조각에서 잘렸을
> 수 있는 마지막 배열 원소를 버리고 항목 스키마 개별 검증 후 같은 index에 `final:false` 재전송,
> 항목 목록은 언제나 최종본의 접두). FE는 도착분을 실제 컴포넌트로
...
| **3. 품질 루프** | 쓰레드 히스토리 기반 개인화, 스튜디오 평가 데이터를 프롬프트 회귀 테스트로, ~~부분 스트리밍(v2 SSE)~~(완료 — §4-1) | 고도화 |

### 9-1. 뼈대/검색 2단계 병렬 생성 + 조기 확정 (2026-08 도입)

계획 생성은 두 LLM 호출의 병렬이다: ① **뼈대**(`PLAN_SKELETON_SYSTEM`, 검색 없음·effort
medium) — headline·summary·**단계 안내(guide) 2~3개**·steps와 상품/콘텐츠 섹션 "자리"
(제목·기준 reason만, 구체 상품명·콘텐츠 제목 금지)를 수 초 안에 스트리밍. ② **검색**
(`PLAN_PRODUCTS_SYSTEM`, 웹 검색 포함·high) — products 섹션 1~2개 + **참고 콘텐츠
(contents — 웹 게시글·영상) 섹션 0~1개**를 생성, 그라운딩 후 자리를 채운다.

- **조기 확정(`skeleton` SSE 이벤트, 2026-08)**: 뼈대 최종 검증이 끝나는 즉시 완성 텍스트
  섹션 + 자리(null·pending 인덱스)를 FE에 보낸다. FE(LivePlayer)는 이 시점에 계획을 확정
  렌더로 전환하고(planKey·설문 잠금·평가 초기화 포함), 자리에는 로딩 카드(`livePending`)를
  둔다 — **웹 검색이 전체 계획 렌더를 막지 않는다**. 이후 `section` 이벤트가 자리를 비동기로
  채우고 result(권위·저장 기준)가 마감한다. 조기 확정 뒤의 status(검색 진행)는 자리 로딩
  카드 문구로만 흐른다. FE는 pending이 남아 있는 동안 재생성 트리거를 막는다(서버 저장 경합
  방지). 구버전 FE는 skeleton 이벤트를 무시하고 result 확정으로 자연 강등된다
- **병합·인덱스 규칙은 `@ddak/pipeline guards/merge.ts`** (스트리밍과 최종 병합이 같은 배정기
  `PlanPlacer`, 2026-09 개정): 뼈대 비자리 섹션은 뼈대 인덱스 그대로. 검색 섹션은 kind별
  도착 순서대로 **단계 묶음(guide + 바로 뒤 자리들)** 중 제목(+reason 0.3 가중)이 묶음
  텍스트(안내 제목·서브타이틀·본문 + 자리 제목·기준)와 가장 닮은 곳으로 간다(글자 2-gram
  덮임률, 공통 2-gram 은 묶음 간 문서 빈도로 감쇠, 빈자리 있는 묶음·직전보다 뒤 묶음 +0.05,
  같은 kind 의 직전 섹션이 간 묶음 이전으로는 못 간다. 닮은 정도 0.04 미만이면 옛 규칙 —
  첫 빈자리 → 아직 안 받은 다음 묶음). 고른 묶음에 빈자리가 있으면 그 뼈대 인덱스, 없으면
  스트리밍에선 뼈대 길이 뒤 index(도착 순) + 끼울 위치 `before`(section 이벤트 — FE 가 result 전에도
  그 단계 안에 그린다, 2026-09-18), 최종 병합에선 그 묶음에 끼운다(콘텐츠는 콘텐츠 자리 뒤·없으면
  안내 바로 뒤 = 상품 앞, 상품은 연속 구간 끝 — steps 뒤에 매달리지 않는다). 뼈대 자체도
  `orderSkeletonSlots` 로 정규화한다 — 첫 안내 앞·steps 뒤에 둔 자리를 단계 구간 안으로 옮기고
  구간 안에서 콘텐츠 → 상품 순 — 어느 경로로든 추천 카드는 단계 컴포넌트 밖에 서지 않는다. 옛 "kind별 k번째 자리"
  규칙은 뼈대(5a)와 검색(5b·5c)이 병렬이라 자리 수·단계 배치가 어긋나면 첫 단계 상품이 뒤
  단계 자리로 밀리고 둘째 섹션이 steps 뒤에 붙었다. 자리 인덱스는 뼈대 **최종 검증본** 기준으로
  확정하고, 먼저 도착한 검색 섹션은 뼈대 완료까지 대기열에 쌓았다 내보낸다 (실측상 뼈대가 항상
  먼저 끝난다). 증분 조각은 title·reason 이 닫힌 뒤에만 나가므로(partial.ts) 배정이 흔들리지 않는다
- 검색 단계 실패는 계획을 죽이지 않는다 — 상품·콘텐츠 없는 계획을 반환하고 로그만 (뼈대 실패는 error)
- llmMeta는 usage 합산 + latency max + `phases`(skeletonMs·productsMs)로 결합 저장
- 프롬프트 캐시는 단계별 시스템 프롬프트가 각자 적중 (둘 다 바이트 고정)
