...
- **병합·인덱스 규칙은 `plan-merge.ts`** (스트리밍과 최종 병합이 같은 규칙): 뼈대 비자리
  섹션은 뼈대 인덱스 그대로, kind별 k번째 검색 섹션 = 같은 kind k번째 자리 인덱스(자리
  초과분은 뼈대 길이 뒤, 도착 순). 자리 인덱스는 뼈대 **최종 검증본** 기준으로 확정하고,
  먼저 도착한 검색 섹션은 뼈대 완료까지 대기열에 쌓았다 내보낸다 (실측상 뼈대가 항상 먼저 끝난다)
