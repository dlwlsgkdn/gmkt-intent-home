...
- **API에 인증 게이트가 없다.** `apps/tagging-api`는 인증 미들웨어를 갖지 않은 채 공유 운영 데이터(Mongo 카탈로그)에 쓰기 권한을 노출한다. 개발 중엔 바인딩 주소를 기본값 127.0.0.1(루프백)로 제한해 같은 기계에서만 접근 가능하게 했다. 사내 서버에 배포하기 전에 인증 방식(사내 SSO 프록시, API 키 검증 등)을 반드시 결정하고 구현할 것.
