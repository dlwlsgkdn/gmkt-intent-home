...
퓨전이 주는 `nodejs:<메이저>-<리비전>` 태그 중에서 고른다(리비전은 이미지 빌드 연월이다).

| 후보 | 판정 |
|---|---|
| `22-2602` | **권장.** 개발 환경과 같은 메이저이고 리비전도 가장 최신이다 |
| `24-2506` | 가능. 2026-09-07 실측으로 빌드 6종·테스트 25건·bff E2E 전부 통과했다. 다만 아래 npm 11 차이를 감수해야 한다 |
| `22-2506` | 가능하지만 `22-2602`가 있는데 8개월 낡은 리비전을 고를 이유가 없다(그만큼의 OS·Node 보안 패치가 빠진다) |
| `18-*`, `16-*` | **불가.** NestJS 11이 Node 20+를 요구한다 |

**Node 24를 고른다면 알아둘 것 둘** (2026-09-07 실측):

1. **npm 11이 의존성 install 스크립트를 기본 차단한다.** `esbuild`·`fsevents` 등의 postinstall이
   실행되지 않고 경고만 남는다. 우리 경우엔 무해했다 — 요즘 esbuild는 플랫폼별 패키지로 오지
   실행 스크립트에 의존하지 않는다. 빌드가 실패하면 여기부터 의심하고,
   `npm install-scripts approve <pkg>`로 개별 허용한다.
2. **`node --test`의 기본 리포터가 TAP에서 spec으로 바뀐다.** 테스트와 종료코드는 정상이지만
   출력이 `# tests 25`에서 `ℹ tests 25`로 바뀌어, TAP을 파싱하는 CI 스크립트는 조용히 빈 결과를
   본다.

위 실측은 macOS arm64에서 한 것이라 UBI 리눅스에서의 최종 확인은 첫 S2I 빌드가 된다. 다만
**Node 버전 때문에 막힐 이유는 없다**는 것은 근거를 갖고 말할 수 있다.
