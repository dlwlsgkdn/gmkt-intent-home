 ...
 * 대표 3개(CASE A/B/C)만 평가하고, 거기서 나온 피드백을 "패턴"으로 삼아
 * 나머지 케이스들의 같은 자리에서 같은 문제를 찾아 고치게 한다.
 *
 * 씨앗(seed) = 대표 케이스의 피드백 + 그 컴포넌트의 현재 필드 값(모범 예시 —
 * 사람이 이미 고쳐 둔 상태라면 그 자체가 정답 예시가 된다).
 * 대상(target) = 대표를 제외한 케이스들에서 씨앗과 같은 타입의 컴포넌트 필드.
 *
...
/* 대표 케이스에서 씨앗 수집 — 컴포넌트 피드백(+현재 값 모범 예시)과 케이스 코멘트 */
export function collectPropagationSeeds(planCases = []) {
  const representatives = evaluationCasesFor(planCases)
  const componentSeeds = []
  const caseNotes = []
  representatives.forEach((planCase) => {
    const evaluation = normalizeCaseEvaluation(planCase.evaluation)
    if (evaluation.feedback.trim()) {
      caseNotes.push({ slot: evaluation.slot, caseName: planCase.name, feedback: evaluation.feedback.trim() })
    }
    componentEvaluationStructureForCase(planCase).forEach((section) => {
      section.components.forEach((component) => {
        ...
        componentSeeds.push({
          ...
          // 대표 케이스의 현재 값 — 이미 고쳐진 상태라면 그대로 "정답 예시"가 된다
  ...
  return { componentSeeds, caseNotes, representativeIds: representatives.map((planCase) => planCase.id) }
}

/* 전파 대상: 대표를 제외한 케이스들에서, 씨앗과 같은 타입 컴포넌트의 편집 가능 필드.
   컴포넌트 씨앗이 없고 케이스 코멘트만 있으면 모든 평가 대상 타입을 포함한다. */
export function propagationTargets(planCases, seeds) {
  const seedTypes = new Set(seeds.componentSeeds.map((seed) => seed.type))
  const representativeIds = new Set(seeds.representativeIds)
  return planCases
    .filter((planCase) => !representativeIds.has(planCase.id))
    .map((planCase) => ({
      caseId: planCase.id,
      name: planCase.name,
...
export function buildPropagationRequest({ scenario, seeds, targets }) {
  return {
    ...
    seeds: {
      caseNotes: seeds.caseNotes,
      components: seeds.componentSeeds.map((seed) => ({
        fromCase: `CASE ${seed.slot} · ${seed.caseName}`,
        type: seed.type,
...
export function buildPropagationPrompt(request) {
  return [
    ...
    '대표 케이스 3개를 사람이 평가해 남긴 피드백(seeds)이 있습니다.',
    ...
    '2. seeds.components[].exemplar는 대표 케이스의 현재 값(모범 예시)입니다. 문체·구조·정보 밀도를 이 수준으로 맞추되,',
    '   각 케이스의 조합 특성이 드러나게 표현은 달리합니다. 예시를 그대로 복사하지 않습니다.',
