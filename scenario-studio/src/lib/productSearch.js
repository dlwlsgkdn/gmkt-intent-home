/*
 * 웹 검색으로 추천 상품 후보 만들기.
 * 시나리오 목업/데이터셋용이라 "실제로 존재하는 상품명"을 확보하는 게 목적이고,
 * 결과는 카탈로그 입력란을 채우기만 한다(생성에 바로 들어가지 않음) — 사용자가 검토·수정한 뒤 사용.
 * 가격은 검색 시점 스니펫 기준이라 정확하지 않을 수 있다.
 */
...
export function buildProductSearchPrompt({ query, persona, notes, count = DEFAULT_SEARCH_COUNT }) {
  return [
    ...
    '',
    '규칙:',
    '1. web_search로 실제 판매 중인 상품을 찾습니다. 상품명과 브랜드는 검색으로 확인한 실제 이름을 씁니다.',
