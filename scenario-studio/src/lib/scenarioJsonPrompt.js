...
- 계획 케이스 items는 보통 이 순서입니다: surveySummary → planTitle → noticeCard → (planStep → hscroll(+productCard 자식)) 반복 → ctaBar.
...
export function buildScenarioDbPrompt({
  ...
  return [
    ...
    catalog
      ...
      : '## 상품\n실제로 판매되는 상품을 조사해 사용하세요. 웹 검색을 쓸 수 있으면 사용하고, 확인되지 않은 가격은 비워 두세요.',
    '',
    '## 출력할 JSON 형태',
    SHAPE_DOC,
    '',
