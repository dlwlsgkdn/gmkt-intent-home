 ...
 * 스튜디오(#ops 파이프라인 탭)가 이 카탈로그를 읽어 노드를 조립·렌더하므로, 새 단계는 여기 등록이 시작점이다.
...
export const PIPELINE_STAGES: PipelineStageDef[] = [
  {
    ...
    kind: 'deterministic',
    status: 'active',
    note: '한 줄 발화가 유일한 씨앗. 명백히 쓸 수 없는 발화(내용 2자 미만·막연 패턴)는 LLM 호출 전에 되돌린다 (guards/objective — 과차단 방지를 위해 보수적).',
  },
  {
    id: 'intent',
    no: '1',
    ...
    effort: 'low',
    status: 'active',
    note: '발화 → 7개 의도 템플릿 + 목적·시점·대상 (구조화 출력). 결과는 원장 facts(source=intent)로 굳어 가변부 "의도 해석"에 실린다. 실패해도 플로우는 계속(fail-open).',
  },
  {
    id: 'ledger',
    no: '2',
  ...
  },
  {
    ...
    effort: 'low',
    status: 'active',
    note: '설문 페이지 생성 — 꼭 필요한 1~3문항만, 원장(프로필·의도 해석)에 있는 정보는 묻지 않는다. 완료 후 답변 대기 interrupt. (0문항 스킵은 FE 플로우 변경 필요 — 보류)',
  },
  {
    id: 'candidates',
    no: '4',
