...
- 섹션은 **계획의 단계 순서대로** 1~2개다: 앞 단계(준비·고르기) 뒤에는 사용법·튜토리얼·비교 콘텐츠, 실행 단계 뒤에는 후기·응용 콘텐츠가 어울린다. 섹션 제목에 어느 단계에 참고할 콘텐츠인지 드러나게 짓고, reason은 사용자의 답변을 근거로 쓴다.
...
export const PROMPT_DEFS: { id: PromptDefId; label: string; note: string; template: string }[] = [
  ...
  },
  {
    id: 'plan-products',
    label: '계획 상품 생성',
    note: `계획 2단계(웹 검색 포함) — 추천 상품 섹션만 채운다(섹션당 상품 5~8개를 브랜드·가격대(저·중·고)·제형 다양하게, PDP 를 못 찾은 상품은 몰 검색 링크(urlKind=search), 상품명은 프로모션 문구·브랜드 중복 없이, 가격 미확인은 0, 카탈로그는 용도가 맞을 때만 섹션당 최대 3개 — v22). 상품마다 매칭 평가(skin·concern·preference 1~5 + 근거)를 매긴다. 매칭율(%)은 검증 게이트가 가중 합산으로 계산한다(@ddak/pipeline guards/match.ts). 참고 콘텐츠는 5c 프롬프트가 따로 만든다. ${CATALOG_PLACEHOLDER} 자리표시자가 상품 카탈로그 목록으로 치환되므로 지우지 말 것. {{CRITERIA}}는 지식 KV로 치환된다.`,
    template: PLAN_PRODUCTS_SYSTEM,
  },
  {
    id: 'plan-contents',
    label: '계획 참고 콘텐츠 생성',
    note: '계획 5c(웹 검색 포함, 상품과 분리 — v22) — 참고 콘텐츠 섹션 1~2개를 단계 순서대로 만든다: 영상·게시글 검색 각 1회 이상, 섹션당 3~6개, 출처 우선순위·같은 출처 2개까지·최근 3년, 항목마다 고른 이유(why). {{CRITERIA}}는 지식 KV로 치환된다.',
