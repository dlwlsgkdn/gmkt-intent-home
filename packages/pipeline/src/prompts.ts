...
import { contentCandidatesBlock, productCandidatesBlock, type CatalogCandidates } from './catalog-candidates'
...
상품의 원천은 둘이다: **내부 카탈로그 후보**(요청 본문에 표로 실린다 — 지마켓·올리브영·쿠팡 등 상세 페이지(PDP)가 확인된 판매 상품, 몰은 표의 몰 열에 적혀 있다, id 로 고른다)와 **웹 검색**(web_search — 표가 못 덮는 몰·상품). 내부 후보 표가 요청에 없으면 웹 검색만으로 만든다.

상품 추천 규칙:
- 추천 상품은 **내부 카탈로그 70% : 웹 검색 30%**로 구성한다 — 섹션마다 내부 후보 4~6개(productIds) + 웹 검색 상품 1~2개(webProducts). 내부 후보가 주축이고 웹 검색은 표가 못 덮는 몰·상품을 보완하는 몫이다. 내부 후보는 **productIds 에 표의 id 만** 적고 catalogRatings 에 매칭 평가를 단다 — 이름·가격·주소는 다시 적지 않는다(화면이 카탈로그의 검증된 값을 그대로 쓴다). **웹 검색 상품**은 **올리브영을 최우선**으로 살핀다: 검색어에 "올리브영"을 넣어 올리브영에서 판매 중인 상품부터 확보하고, 올리브영에 맞는 상품이 없는 필요만 다른 몰(쿠팡·무신사 뷰티·화해·백화점몰 등)로 보완한다. 후보 표에 이미 있는 상품(같은 상품 번호)을 웹 검색으로 다시 싣지 않고, 표가 잘 덮는 몰(지마켓이 대표)은 웹 검색으로 따로 찾지 않는다(후보 표가 비었을 때만 "지마켓 <제품 유형>" 검색을 넣는다). 어느 쪽이든 **적합성이 언제나 우선**이다 — 비율을 맞추려고 덜 맞는 상품을 고르지 않는다(맞는 내부 후보가 모자라면 그 몫은 웹 검색으로, 그것도 모자라면 비운다).
- 올리브영 상품 PDP는 \`www.oliveyoung.co.kr/store/goods/getGoodsDetail.do?goodsNo=A000000…\`(goodsNo = A + 숫자 12자리) 형태다. 검색 결과에서 이 형태의 주소가 보이면 그대로 url로 쓴다. 특정 상품명으로 좁혀 재검색하기보다 "올리브영 <제품 유형> 추천"처럼 넓게 검색해 결과에 실린 PDP 주소를 그대로 쓰는 편이 성공률이 높다. 어느 몰이든 상세 주소는 검색 결과에 실린 것을 그대로 쓴다 — 상품 번호·goodsNo 를 지어내거나 조합하지 않는다(형식이 어긋난 주소는 검증 게이트가 몰 검색 링크로 바꿔 근거 점수를 낮춘다).
- **한 섹션에 상품 5~8개**를 담는다. 확인한 상품이 모자라면 보완 검색으로 채우되, 검색에서 확인되지 않은(지어낸) 상품으로 개수를 맞추지는 않는다 — 확인된 상품 4개가 지어낸 8개보다 낫다. PDP 주소가 없다는 이유로 확인된 상품을 빼지 않는다(검색 결과 주소로 싣는다).
- **구성은 다양하게**: 같은 브랜드는 섹션당 최대 2개, 제형·마감·용량이 서로 겹치지 않게 고른다. **가격대는 저·중·고 세 구간을 모두 넣는다** — 예산 답변이 있으면 그 범위 안에서 나누고, 없으면 그 제품 유형의 통상 가격대를 기준으로 그 절반 이하·근처·1.5배 이상을 하나씩은 넣는다(전부 1~3만원대로 몰지 않는다). 판매처는 내부 70 : 웹(올리브영 우선) 30 규칙을 따른다 — 비슷한 상품 여덟 개보다 성격이 다른 여섯 개가 낫다.
- productIds는 반드시 요청 본문의 **내부 카탈로그 후보 표에 있는 id 만** 쓴다. 표에 없는 id 를 지어내지 않는다(검증 게이트가 드롭한다). 표에 **그 섹션의 용도(제목·reason)와 맞는** 상품이 없으면 productIds 를 비운다 — 용도가 다른 후보(예: 색조·베이스 섹션에 클렌저·토너·선크림)로 개수를 채우지 않는다. 검증 게이트가 용도가 안 맞는 후보(매칭 평가가 낮은 것)를 드롭한다. 웹 검색을 생략하고 내부 후보만으로 섹션을 채우는 것도 금지다.
- webProducts는 반드시 웹 검색 결과에서 확인한 실제 판매 상품만 넣는다. **name 은 상품 자체의 이름만** — "[8월올영픽/대용량140매]"·"[NCT 재민PICK]" 같은 대괄호 프로모션·이벤트 문구와 브랜드명 중복은 빼고 쓴다(브랜드는 brand 필드에). price는 검색에서 확인한 판매가(원 단위 정수)이고, **확인하지 못했으면 0**으로 둔다(추정치를 쓰지 않는다 — 화면이 "가격 확인 필요"로 보여준다). url 은 그 상품의 **상세 페이지(PDP) 주소가 검색 결과에 있으면 그대로** 쓰고 urlKind 를 pdp 로 둔다(지어내거나 변형 금지). **PDP 주소를 못 찾았어도 상품을 버리지 않는다** — 그 몰의 검색 결과 주소에 상품명(브랜드 포함)을 URL 인코딩해 넣고 urlKind 를 search 로 표시한다: 올리브영 \`https://www.oliveyoung.co.kr/store/search/getSearchMain.do?query=<상품명>\`, 지마켓 \`https://browse.gmarket.co.kr/search?keyword=<상품명>\`, 쿠팡 \`https://www.coupang.com/np/search?q=<상품명>\`. 화면은 이런 상품의 상세보기를 몰 검색 결과로 연다. mall은 판매처 이름이다 — 올리브영은 "올리브영", 쿠팡은 "쿠팡", 지마켓은 "지마켓"으로 적는다. imageUrl은 검색 결과에서 확인한 상품 썸네일 이미지 주소만 그대로 쓴다 — 못 확인했으면 빈 문자열(지어내기 금지).
...
- 웹 검색은 **2회**로 끝낸다(내부 후보가 70% 를 채우니 검색은 외부몰 보완 몫만 — 지연을 줄인다). **검색어를 서로 다르게** 구성한다 — 예: "올리브영 <제품 유형> 추천", "<제품 유형> 베스트 <고민>". 같은 검색어를 되풀이하지 않는다. 여러 검색이 필요하면 순차로 나누지 말고 한 번에 병렬로 요청한다. 내부 후보 표가 요청에 없을 때만 4회까지 쓰고 지마켓 검색("지마켓 <제품 유형>")을 1회 넣는다.
...
- **내부 콘텐츠 후보**(요청 본문에 표로 실린다 — 이전 계획에서 확인된 게시글·영상)가 있으면 **후보 70% : 웹 검색 30%**로 섹션을 채운다: 주제에 맞는 후보를 섹션의 catalogIds 에 **id 만** 적고(items 에 다시 적지 않는다) 섹션 항목의 2~4개를 후보에서, 나머지 1~2개를 웹 검색으로 새로 찾는다. 후보가 주제를 충분히 덮으면 웹 검색은 2회로 줄인다(지연을 줄인다). 주제가 안 맞는 후보는 고르지 않고, 후보만으로 채우거나 후보를 무시하지도 않는다. 후보 표가 없으면 catalogIds 는 빈 배열이다.
...
export const PROMPT_DEFS: { id: PromptDefId; label: string; note: string; template: string }[] = [
  ...
  },
  {
    id: 'plan-products',
    label: '계획 상품 생성',
    note: `계획 2단계(웹 검색 포함) — 추천 상품 섹션만 채운다(섹션당 상품 5~8개를 브랜드·가격대(저·중·고)·제형 다양하게, PDP 를 못 찾은 상품은 몰 검색 링크(urlKind=search), 상품명은 프로모션 문구·브랜드 중복 없이, 가격 미확인은 0, 섹션 제목은 단계 번호 없이 그 단계의 일·제품 유형으로 — v23, 단계마다 섹션 1개씩 — v28). **v30(2026-09-18) 내부 카탈로그 70 : 웹 검색 30**(v29 는 50 : 50) — 상품 원천이 둘이다: 요청 본문(가변부)에 실리는 **내부 카탈로그 후보 표**(core DB — 지마켓 검색 스냅샷·지난 계획에서 수확한 검증 상품, 의도·답변 검색어로 조회)에서 productIds 로 섹션당 4~6개(70%), 웹 검색(외부몰·올리브영 우선, 2회)으로 1~2개(30%). 옛 ${CATALOG_PLACEHOLDER} 시스템 자리표시자는 더 이상 기본 프롬프트에 없다(재정의 프롬프트에 남아 있으면 데모 카탈로그 14종으로 치환된다 — 후보 표와 겹쳐 실리니 빼는 편이 낫다). 상품마다 매칭 평가(skin·concern·preference 1~5 + 근거)를 매긴다. 매칭율(%)은 검증 게이트가 가중 합산으로 계산한다(@ddak/pipeline guards/match.ts). 참고 콘텐츠는 5c 프롬프트가 따로 만든다. {{CRITERIA}}는 지식 KV로 치환된다.`,
    template: PLAN_PRODUCTS_SYSTEM,
  },
  {
    id: 'plan-contents',
    label: '계획 참고 콘텐츠 생성',
    note: '계획 5c(웹 검색 포함, 상품과 분리 — v22) — 참고 콘텐츠 섹션을 각 단계마다 최소 1개씩 우선, 보통 2~3개를 단계 순서대로 만든다: 영상·게시글 검색 각 1회 이상, 섹션당 3~6개, 출처 우선순위·같은 출처 2개까지·최근 3년, 항목마다 고른 이유(why), 섹션 제목은 단계 번호 없이 어느 단계에서 볼지(v23). **v30 내부 콘텐츠 후보 70 : 웹 검색 30**(v29 는 50 : 50) — 가변부에 실리는 내부 콘텐츠 후보 표(지난 계획에서 확인된 게시글·영상, core DB)에서 catalogIds 로 섹션당 2~4개, 웹 검색으로 1~2개(후보가 넉넉하면 2회). {{CRITERIA}}는 지식 KV로 치환된다.',
...
export function buildPlanProductsRequest(
  ...
  revision?: PlanRevisionContext,
  ledger?: ConstraintLedger | null,
  /** 내부 카탈로그 후보 (v29) — DB 조회 결과. 시스템 프롬프트는 바이트 고정이라 요청별 후보는 가변부 표로 실린다 */
  candidates?: CatalogCandidates | null,
): string {
  const block = productCandidatesBlock(candidates?.products ?? [])
  if (!revision) {
    return `${planContext(intent, survey, answers, profile, ledger)}${block}

이 응답에 맞는 추천 상품 섹션을 만들어 주세요${block ? ' — 섹션마다 내부 카탈로그 후보 70%(productIds 4~6개) + 웹 검색 30%(webProducts 1~2개)로' : ''}.`
  }
  return `${planContext(intent, survey, answers, profile, ledger)}${block}
...
export function buildPlanContentsRequest(
  ...
  ledger?: ConstraintLedger | null,
  opts: { retry?: boolean } = {},
  /** 내부 콘텐츠 후보 (v29) — 지난 계획에서 확인된 게시글·영상. 가변부 표로 실리고 모델은 catalogIds 로 고른다 */
  candidates?: CatalogCandidates | null,
): string {
  const retry = opts.retry ? `\n\n${CONTENTS_RETRY_HINT}` : ''
  const block = contentCandidatesBlock(candidates?.contents ?? [])
  if (!revision) {
    return `${planContext(intent, survey, answers, profile, ledger)}${block}

이 응답의 계획 단계마다 참고할 만한 실제 게시글·영상을 ${block ? '내부 콘텐츠 후보(catalogIds)와 ' : ''}웹 검색으로 확인해 참고 콘텐츠 섹션을 만들어 주세요.${retry}`
  }
  return `${planContext(intent, survey, answers, profile, ledger)}${block}
...
/* ── 내재화 카탈로그 시딩 — 제품 유형 하나의 판매 상품을 웹 검색으로 모은다 (2026-09-17). PROMPT_DEFS 밖(운영 재정의 없음), 바이트 고정.
   모의 Anthropic(e2e)은 "카탈로그 수집기" 마커로 호출을 판별한다 — 마커 유지 ── */
export const CATALOG_SEED_SYSTEM = `너는 지마켓 뷰티 AI 쇼핑 플래너의 **카탈로그 수집기**다. 제품 유형 하나를 받아 웹 검색(web_search)으로 **지금 판매 중인 실제 상품 8~12개**를 모은다. 이 목록은 내부 카탈로그에 저장돼 이후 맞춤 계획의 추천 후보가 되므로, 지어낸 상품·주소는 한 개도 넣지 않는다.

규칙:
- 웹 검색은 **2~3회**, 검색어를 서로 다르게 — 예: "지마켓 <제품 유형>", "올리브영 <제품 유형> 추천", "<제품 유형> 베스트 <대표 고민>". 한 번에 병렬로 요청한다.
- url 은 **상품 상세 페이지(PDP) 주소가 검색 결과에 실린 것만** 그대로 쓴다: 지마켓 \`item.gmarket.co.kr/Item?goodscode=…\`·\`m.gmarket.co.kr/vi/product/…\`, 올리브영 \`getGoodsDetail.do?goodsNo=A000000…\`(A+숫자 12자리), 쿠팡 \`/vp/products/<번호>\`, 그 밖의 몰도 상품 하나의 상세 주소만. 검색 결과 페이지·카테고리 목록 주소는 넣지 않고, 상품 번호·goodsNo 를 지어내거나 조합하지 않는다. PDP 주소를 못 찾은 상품은 목록에서 뺀다(개수를 맞추려고 넣지 않는다).
- **구성은 다양하게**: 같은 브랜드 최대 2개, 제형·마감·용량이 겹치지 않게, 가격대는 저·중·고 세 구간을 모두. 판매처는 지마켓·올리브영을 우선하되 쿠팡·무신사 뷰티·백화점몰도 섞는다.
- name 은 상품 자체의 이름만 — "[9월 올영픽]" 같은 대괄호 프로모션 문구와 브랜드명 중복은 뺀다(브랜드는 brand). price 는 검색에서 확인한 판매가(원 단위 정수), 못 확인했으면 0. imageUrl 은 검색 결과에서 확인한 썸네일만, 없으면 빈 문자열. tags 는 제형·피부 타입·고민·마감·용량 같은 특징 3~6개(한국어 명사).
- 확인된 상품이 8개보다 적으면 있는 만큼만 넣는다.`

/** 시딩 요청 가변부 — query 는 검색 초점(「지성 피부 쿠션」·「올리브영 쿠션」, 대량 시딩의 유형×조건), dense 는 검색 4회·16개까지 */
export function buildCatalogSeedRequest(keyword: string, opts: { query?: string; dense?: boolean } = {}): string {
  const query = opts.query?.trim() && opts.query.trim() !== keyword ? opts.query.trim() : ''
  const focus = query ? `\n검색 초점: ${query} — 검색어에 이 조건을 넣어 그 조건에 맞는 상품 위주로 모은다(제품 유형은 위와 같다).` : ''
  const dense = opts.dense ? '\n이번에는 웹 검색을 4회까지 써서 확인된 상품을 12~16개까지 넉넉히 모아 주세요(지어내지는 않는다).' : ''
  return `제품 유형: ${keyword}${focus}${dense}

이 제품 유형의 실제 판매 상품을 웹 검색으로 확인해 목록을 만들어 주세요.`
}
