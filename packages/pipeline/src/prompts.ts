...
- 섹션 구성: 단계마다 [단계 안내(guide) → 그 단계에 참고할 콘텐츠 자리(contents — 제목·기준만) → 그 단계의 상품 자리(products — 제목과 "고를 기준" reason만)] 묶음을 단계 순서대로 이어 가고, 마지막에 사용 순서(steps)로 닫는 것이 기본 골격이다. **콘텐츠 자리와 상품 자리는 반드시 그것이 필요한 단계 안내 바로 뒤**에 두고, **한 단계 안에서는 콘텐츠 자리가 상품 자리보다 앞**이다(먼저 보고 배운 뒤 고른다) — 콘텐츠 카드는 계획 끝이 아니라 단계 본문 사이에 끼워 넣는 것이다. 상품이 필요 없는 단계는 상품 자리를, 참고할 게 없는 단계는 콘텐츠 자리를 생략한다(상품 자리 총 1~2개, 콘텐츠 자리 총 1~2개 — 예: 준비 단계 뒤엔 사용법·튜토리얼 영상 자리, 핵심 실행 단계 뒤엔 비교·후기 게시글 자리). **그 단계에서 고르거나 사는 제품이 있으면 첫 단계라도 상품 자리를 둔다**(준비 단계의 프라이머·패드처럼) — 상품 자리는 보통 준비·고르기 단계 뒤 하나, 실행·마무리 단계 뒤 하나다. 자리의 title·reason 에는 그 단계에서 고를 제품 유형과 기준을 구체적으로 적는다 — 검색 단계의 결과를 어느 단계에 붙일지 이 문구와 단계 안내를 대조해 정한다. 검색 단계가 이 자리들을 실제 상품·게시글·영상으로 채운다. 상품·콘텐츠 자리를 단계들과 떨어뜨려 끝에 몰아 두지 않는다.
