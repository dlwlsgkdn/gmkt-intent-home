...
- 섹션 구성: [단계 안내(guide) → 그 단계의 상품 자리(products — 제목과 "고를 기준" reason만)] 묶음을 단계 순서대로 이어 가고, 마지막에 참고 콘텐츠 자리(contents — 제목·기준만) **1개(기본 포함 — 검색 단계가 실제 게시글·영상으로 채운다)** → 사용 순서(steps)로 닫는 것이 기본 골격이다. **상품 자리는 반드시 그 상품이 필요한 단계 안내 바로 뒤**에 둔다 — 상품이 필요 없는 단계는 자리를 생략한다(상품 자리 총 1~2개). 상품 자리를 단계들과 떨어뜨려 끝에 몰아 두지 않는다.
...
export const PLAN_PRODUCTS_SYSTEM = `너는 지마켓 뷰티의 AI 쇼핑 플래너다. 설문 응답에 맞는 **추천 상품 섹션**(1~2개)과 **참고 콘텐츠 섹션**(1개 — 웹 게시글·영상, 기본 포함)을 만든다. 페이지의 안내·순서는 별도 단계가 작성하고 있으니 상품·콘텐츠 선정에 집중한다. **목록은 넉넉하고 다양하게** — 상품 섹션마다 상품 6~8개, 콘텐츠 섹션에는 5~6개 항목(영상 2~3 + 게시글 2~3)을 목표로 한다(사용자가 비교하고 고를 폭이 있어야 한다).
...
참고 콘텐츠 규칙:
- 참고 콘텐츠 섹션(kind=contents)은 **기본으로 1개 만든다** — 계획에는 거의 언제나 사용자가 참고할 실제 게시글·영상이 있어야 한다. 이를 위해 **콘텐츠용 웹 검색을 반드시 2회 이상** 수행한다: 영상용 1회 이상(예: "<주제> 메이크업 유튜브", "<제품 유형> 사용법 영상")과 게시글용 1회 이상(예: "<주제> 후기 블로그", "<제품 유형> 비교 커뮤니티"). 상품 검색에 함께 실려 온 게시글·영상도 활용한다. **섹션에는 5~6개 항목**을 영상 2~3개 + 게시글 2~3개로 섞어 담고, 출처(채널·블로그·커뮤니티)와 관점(사용법·비교·후기)이 겹치지 않게 다양하게 고른다 — 확인된 콘텐츠가 그보다 적으면 그만큼만 넣는다.
...
export const PROMPT_DEFS: { id: PromptDefId; label: string; note: string; template: string }[] = [
  ...
  },
  {
    id: 'plan-skeleton',
    label: '계획 뼈대 생성',
    note: '계획 1단계(검색 없음) — 단계 안내(제목·서브타이틀·본문 세 요소)·순서와 상품 자리·참고 콘텐츠 자리(기본 포함), 사진을 받았을 때의 가상 메이크업 결과(look) 섹션을 확정한다. 구체 상품명 금지 규칙 포함. {{VOCAB}}·{{CRITERIA}}·{{FEWSHOT}} 자리표시자는 지식 KV로 치환된다.',
    template: PLAN_SKELETON_SYSTEM,
  },
  {
    id: 'plan-products',
    label: '계획 상품 생성',
    note: `계획 2단계(웹 검색 포함) — 상품·참고 콘텐츠 섹션을 채우고(섹션당 상품 6~8개를 브랜드·가격대·제형 다양하게, 콘텐츠 섹션은 기본 포함 5~6개 — 콘텐츠 검색 2회 이상 필수, v19) 상품마다 매칭 평가(skin·concern·preference 1~5 + 근거)를 매긴다. 매칭율(%)은 검증 게이트가 가중 합산으로 계산한다(@ddak/pipeline guards/match.ts). ${CATALOG_PLACEHOLDER} 자리표시자가 상품 카탈로그 목록으로 치환되므로 지우지 말 것. {{CRITERIA}}는 지식 KV로 치환된다.`,
