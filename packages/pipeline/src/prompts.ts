...
상품 추천 규칙:
- 추천 상품은 **내부 카탈로그 절반 : 웹 검색 절반**으로 구성한다 — 섹션마다 내부 후보 3~4개(productIds) + 웹 검색 상품 3~4개(webProducts). 내부 후보는 **productIds 에 표의 id 만** 적고 catalogRatings 에 매칭 평가를 단다 — 이름·가격·주소는 다시 적지 않는다(화면이 카탈로그의 검증된 값을 그대로 쓴다). **웹 검색 상품**은 **올리브영을 최우선**으로 살핀다: 검색어에 "올리브영"을 넣어 올리브영에서 판매 중인 상품부터 확보하고, 올리브영에 맞는 상품이 없는 필요만 다른 몰(쿠팡·무신사 뷰티·화해·백화점몰 등)로 보완한다. 후보 표에 이미 있는 상품(같은 상품 번호)을 웹 검색으로 다시 싣지 않고, 표가 잘 덮는 몰(지마켓이 대표)은 웹 검색으로 따로 찾지 않는다(후보 표가 비었을 때만 "지마켓 <제품 유형>" 검색을 넣는다). 어느 쪽이든 **적합성이 언제나 우선**이다 — 비율을 맞추려고 덜 맞는 상품을 고르지 않는다(맞는 내부 후보가 모자라면 그 몫은 웹 검색으로, 그것도 모자라면 비운다).
...
- **구성은 다양하게**: 같은 브랜드는 섹션당 최대 2개, 제형·마감·용량이 서로 겹치지 않게 고른다. **가격대는 저·중·고 세 구간을 모두 넣는다** — 예산 답변이 있으면 그 범위 안에서 나누고, 없으면 그 제품 유형의 통상 가격대를 기준으로 그 절반 이하·근처·1.5배 이상을 하나씩은 넣는다(전부 1~3만원대로 몰지 않는다). 판매처는 내부 절반 : 웹(올리브영 우선) 절반 규칙을 따른다 — 비슷한 상품 여덟 개보다 성격이 다른 여섯 개가 낫다.
...
- 웹 검색은 **2~3회**로 끝낸다(내부 후보가 절반을 채우니 검색은 외부몰 몫만 — 지연을 줄인다). **검색어를 서로 다르게** 구성한다 — 예: "올리브영 <제품 유형> 추천", "<제품 유형> 베스트 <고민>", "<제품 유형> <제형> 추천 후기". 같은 검색어를 되풀이하지 않는다. 여러 검색이 필요하면 순차로 나누지 말고 한 번에 병렬로 요청한다. 내부 후보 표가 요청에 없을 때만 4회까지 쓰고 지마켓 검색("지마켓 <제품 유형>")을 1회 넣는다.
...
- **내부 콘텐츠 후보**(요청 본문에 표로 실린다 — 이전 계획에서 확인된 게시글·영상)가 있으면 **후보 절반 : 웹 검색 절반**으로 섹션을 채운다: 주제에 맞는 후보를 섹션의 catalogIds 에 **id 만** 적고(items 에 다시 적지 않는다), 나머지 절반은 웹 검색으로 새로 찾는다. 후보가 주제를 충분히 덮으면 웹 검색은 2회로 줄여도 된다(지연을 줄인다). 주제가 안 맞는 후보는 고르지 않고, 후보만으로 채우거나 후보를 무시하지도 않는다. 후보 표가 없으면 catalogIds 는 빈 배열이다.
...
export const PROMPT_DEFS: { id: PromptDefId; label: string; note: string; template: string }[] = [
  ...
  },
  {
    id: 'plan-products',
    label: '계획 상품 생성',
    note: `계획 2단계(웹 검색 포함) — 추천 상품 섹션만 채운다(섹션당 상품 5~8개를 브랜드·가격대(저·중·고)·제형 다양하게, PDP 를 못 찾은 상품은 몰 검색 링크(urlKind=search), 상품명은 프로모션 문구·브랜드 중복 없이, 가격 미확인은 0, 섹션 제목은 단계 번호 없이 그 단계의 일·제품 유형으로 — v23, 단계마다 섹션 1개씩 — v28). **v29(2026-09-17) 내부 카탈로그 50 : 웹 검색 50** — 상품 원천이 둘이다: 요청 본문(가변부)에 실리는 **내부 카탈로그 후보 표**(core DB — 지마켓 검색 스냅샷·지난 계획에서 수확한 검증 상품, 의도·답변 검색어로 조회)에서 productIds 로 절반, 웹 검색(외부몰·올리브영 우선, 2~3회)으로 절반. 옛 ${CATALOG_PLACEHOLDER} 시스템 자리표시자는 더 이상 기본 프롬프트에 없다(재정의 프롬프트에 남아 있으면 데모 카탈로그 14종으로 치환된다 — 후보 표와 겹쳐 실리니 빼는 편이 낫다). 상품마다 매칭 평가(skin·concern·preference 1~5 + 근거)를 매긴다. 매칭율(%)은 검증 게이트가 가중 합산으로 계산한다(@ddak/pipeline guards/match.ts). 참고 콘텐츠는 5c 프롬프트가 따로 만든다. {{CRITERIA}}는 지식 KV로 치환된다.`,
    template: PLAN_PRODUCTS_SYSTEM,
  },
  {
    id: 'plan-contents',
    label: '계획 참고 콘텐츠 생성',
    note: '계획 5c(웹 검색 포함, 상품과 분리 — v22) — 참고 콘텐츠 섹션을 각 단계마다 최소 1개씩 우선, 보통 2~3개를 단계 순서대로 만든다: 영상·게시글 검색 각 1회 이상, 섹션당 3~6개, 출처 우선순위·같은 출처 2개까지·최근 3년, 항목마다 고른 이유(why), 섹션 제목은 단계 번호 없이 어느 단계에서 볼지(v23). **v29 내부 콘텐츠 후보 50 : 웹 검색 50** — 가변부에 실리는 내부 콘텐츠 후보 표(지난 계획에서 확인된 게시글·영상, core DB)에서 catalogIds 로 절반, 웹 검색으로 절반(후보가 넉넉하면 2회). {{CRITERIA}}는 지식 KV로 치환된다.',
...
이 응답에 맞는 추천 상품 섹션을 만들어 주세요${block ? ' — 섹션마다 내부 카탈로그 후보 절반(productIds) + 웹 검색 절반(webProducts)으로' : ''}.`
  }
  return `${planContext(intent, survey, answers, profile, ledger)}${block}
