...
- **한 섹션에 상품 6~8개**를 담는다(최소 5개를 목표로). 검색 결과에서 PDP 를 확인한 상품이 모자라면 보완 검색으로 채우되, 확인되지 않은 상품으로 개수를 맞추지는 않는다 — 검증된 상품 4개가 지어낸 8개보다 낫다.
...
export const PROMPT_DEFS: { id: PromptDefId; label: string; note: string; template: string }[] = [
  ...
  },
  {
    id: 'plan-products',
    label: '계획 상품 생성',
    note: `계획 2단계(웹 검색 포함) — 상품·참고 콘텐츠 섹션을 채우고(섹션당 상품 6~8개를 브랜드·가격대·제형 다양하게, 콘텐츠 섹션은 단계 순서로 1~2개 기본 포함 — 콘텐츠 검색 2회 이상 필수, v20) 상품마다 매칭 평가(skin·concern·preference 1~5 + 근거)를 매긴다. 매칭율(%)은 검증 게이트가 가중 합산으로 계산한다(@ddak/pipeline guards/match.ts). ${CATALOG_PLACEHOLDER} 자리표시자가 상품 카탈로그 목록으로 치환되므로 지우지 말 것. {{CRITERIA}}는 지식 KV로 치환된다.`,
