 ...
 * 계획 생성이 웹 검색만으로 상품·콘텐츠를 찾던 것을 **내부 DB 후보 70% + 웹 검색 30%**로 바꾼다(v29 는 50 : 50, v30 2026-09-18 부터 70 : 30).
 *
...
export function productCandidatesBlock(products: CatalogProduct[]): string {
  ...
  return `\n\n내부 카탈로그 후보 (검증된 판매 상품 — id | 몰 | 브랜드 상품명 | 가격 | 태그):\n${rows.join('\n')}\n이 후보에서 고른 상품은 productIds 에 **id 만** 적는다(이름·가격·주소를 다시 적지 않는다 — 화면이 카탈로그 값을 그대로 쓴다). 섹션마다 상품의 **70%(4~6개)를 이 후보에서**, 나머지 30%(1~2개)를 웹 검색(외부몰)에서 고른다. 후보 중 그 섹션 용도에 맞는 상품이 모자라면 맞는 것만 고르고 웹 검색 몫을 늘린다 — 용도가 다른 후보로 개수를 채우지 않는다.`
}
...
export function contentCandidatesBlock(contents: CatalogCandidates['contents']): string {
  ...
  return `\n\n내부 콘텐츠 후보 (이전 계획에서 확인된 게시글·영상 — id | 종류 | 출처 | 제목 | 부가 정보):\n${rows.join('\n')}\n이 후보에서 고른 콘텐츠는 섹션의 catalogIds 에 **id 만** 적는다(items 에 다시 적지 않는다). 주제에 맞는 후보가 있으면 섹션마다 항목의 **70%(2~4개)는 이 후보에서**, 나머지 30%(1~2개)는 웹 검색으로 새로 찾는다 — 후보만으로 채우거나 후보를 무시하지 않는다. 주제가 안 맞는 후보는 고르지 않는다.`
}
