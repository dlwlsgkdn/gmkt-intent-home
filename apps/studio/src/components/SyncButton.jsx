 ...
 *
 * 빌더 상단바 전용 — 앱에서 유일한 저장 상태 표시·수동 저장 자리다 (홈은 쓰기가
 * 전부 자동 싱크라 저장 UI가 없고, 프로필 버튼 배지로 다운로드 상태만 보여준다).
 * 스튜디오 밖 단발 트랜잭션(프로필·시나리오 생성, 쓰레드 등)은 자동으로 싱크되지만,
 * 빌더의 연속 편집은 이 버튼이 곧 전송 시점이다 — 빌더를 떠나는 발행하기·발행 취소·홈으로도
 * 같은 push 를 부른다(useTopBarActions.pushToServerSoon, 2026-09-16). 상태 점이 미저장(주황)·
