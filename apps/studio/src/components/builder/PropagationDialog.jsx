...
export default function PropagationDialog({ scenario, planCases, onApply, onClose, onToast }) {
  ...
  return (
    <LlmDialogShell
      titleId="sb-prop-title"
      className="sb-gen-dialog"
      label="평가 피드백 → 프롬프트 → 내 AI → 전체 케이스"
      title="피드백 전체 반영"
      description={<>
    ...
    >
      <DialogSteps steps={[
        { label: '씨앗 확인', state: phase === 'setup' ? 'active' : 'done' },
        { label: '프롬프트 왕복', state: phase === 'exchange' ? 'active' : phase === 'review' ? 'done' : null },
        { label: '검토 · 적용', state: phase === 'review' ? 'active' : null },
      ]} />
      ...
      {phase === 'exchange' && (
        <>
          <BatchBar
            ...
            caption={activeBatch.map((target) => target.name).join(' · ')}
            percent={batches.length > 0 ? Math.round((batchIndex / batches.length) * 100) : 0}
            status={`수정안 ${collected.length}개 수집`}
          />
