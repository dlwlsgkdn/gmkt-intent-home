...
import { MIN_ITEM_W, placeChild, reorderTop, topIndexOf } from '../../../lib/builder/geometry.js'

/*
 * 컨테이너(레이아웃 컴포넌트) 안의 자식을 다루는 조작들.
 *
 * 자식은 같은 아이템 배열에 parentId + slot으로 평평하게 저장된다. 최상위 스택 순서의
 * 대상이 아니라 슬롯 순서가 곧 배치이고, w/h는 컨테이너 안 카드의 콘텐츠 크기다.
 * 넣기·꺼내기·슬롯 재정렬이 전부 여기 모여 있다.
 *
 * 캔버스에서의 자식 드래그는 2단계다:
 *   클릭 = 선택 → 컨테이너 안에서 드래그 = 슬롯 재정렬 미리보기
 *   → 포인터가 컨테이너 박스를 벗어나면 자동으로 꺼내져 최상위 순서 드래그로 전환
 */
export function useContainerNesting({
  ...
  const addChild = (type, parentId, at) => {
    ...
    // 최초 배치 때 한 번만 계산해 저장하므로 이후 컨테이너 변화에는 영향받지 않는다.
    ...
    const item = { ...createItem(type, def.defaults), w }
  ...
  const unnestItem = (id) => {
    setItems((prev) => {
      ...
      const { parentId, slot, ...freed } = target
      return reorderTop([...rest, freed], [id], Infinity)
    })
    showToast('레이아웃에서 꺼내 스택 맨 아래에 놓았어요.')
  }
  ...
  const childPointerDown = (event, childId) => {
    ...
    const MARGIN = 18
    let phase = 'idle' // 'idle' → 'reorder'(컨테이너 안) → 'out'(꺼내진 최상위 순서 드래그)
    let lastSlot = -1
    ...
    const popOut = (clientX, clientY) => {
      phase = 'out'
      setItems((prev) => {
        const target = prev.find((item) => item.id === childId)
        if (!target) return prev
        const rest = prev.filter((item) => item.id !== childId)
        const { parentId, slot, ...freed } = target
        return reorderTop([...rest, freed], [childId], Infinity)
      })
      setSelectedIds([childId])
      drag.beginFromPointer(childId, clientX, clientY) // 이후는 최상위 순서 드래그로 이어진다
    }
    ...
    const move = (moveEvent) => {
      ...
      if (phase === 'reorder') {
        ...
        // 컨테이너 밖 → 꺼내서 최상위 순서 드래그로 전환
        setDropTargetId(null)
        setDraggingChildId(null)
        setInsertHint(null)
        popOut(moveEvent.clientX, moveEvent.clientY)
      }
      if (phase === 'out') {
        drag.onDrag(childId, moveEvent.clientX, moveEvent.clientY)
      }
    }
    const up = () => {
  ...
  /* 자식 리사이즈 핸들 — 컨테이너 안 카드의 콘텐츠 크기(w/h) 조절 */
  ...
  /* 레이어 패널 순서 바꾸기 — 최상위는 배열 순서, 컨테이너 자식은 슬롯 교환 */
  const moveLayer = (id, dir) => {
    setItems((prev) => {
      ...
      const index = topIndexOf(prev, id)
      if (index < 0) return prev
      // 이동 아이템을 뺀 나머지 기준으로 한 칸 이동 (reorderTop 규약)
      return reorderTop(prev, [id], index + dir)
    })
  }
  ...
  const dropLayer = (id, targetId, placement) => {
    ...
    // 최상위끼리는 순서만 바꾼다 (자식을 끌어냈으면 최상위로 승격)
    setItems((prev) => {
      const current = prev.find((item) => item.id === id)
      if (!current) return prev
      const { parentId, slot, ...freed } = current
      const withoutSource = prev.map((item) => (item.id === id ? freed : item))
      const rest = withoutSource.filter((item) => !item.parentId && item.id !== id)
      const targetIndex = rest.findIndex((item) => item.id === targetId)
      if (targetIndex < 0) return prev
      return reorderTop(withoutSource, [id], targetIndex + (placement === 'after' ? 1 : 0))
    })
    setSelectedIds([id])
  }
