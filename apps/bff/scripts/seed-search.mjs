...
/* 내재화 카탈로그 대량 시딩 드라이버 (2026-09-17) — 서버 시딩 잡(BFF `/api/admin/catalog/seed-job`, 상태는 core KV)을 시작하거나 이어받아
   `step` 을 반복 호출해 4단위씩(한 라운드) 전진시킨다. 상태·진행·회차 기록·결과는 전부 서버에 있어 **운영 콘솔 「내재화 카탈로그」 카드가 실시간으로
   ...
   BFF 는 Vercel 서버리스라 한 회차(4단위 병렬 한 라운드, 호출당 220초 상한)가 300초 안에 끝나야 한다 — 서버가 회차마다 잠금(270초)을 걸어 콘솔 탭과 이 스크립트가 같은 잡을
