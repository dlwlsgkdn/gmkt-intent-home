...
- 판매처는 골고루: 섹션마다 카탈로그(지마켓) 상품만으로 채우지 말고 웹 상품을 섞고, webProducts끼리는 서로 다른 몰에서 고른다(예: 올리브영·쿠팡·무신사 뷰티·화해·백화점몰 등 — 같은 몰 반복 지양). 단, 몰을 다양하게 하려고 덜 맞는 상품을 고르지는 않는다 — 적합성이 우선이다.
