...
**서버 동기화는 자동 다운로드 + 수동 업로드(빌더) + 트랜잭션 자동 싱크(스튜디오 밖)** (`api/state.js` + `hooks/useWorkspace.js` + `components/SyncButton.jsx`): 접속 시 서버로 하이드레이션하고, 빌더의 연속 편집은 "서버에 저장"으로 올린다 — 빌더 상단바는 SyncButton, 홈은 프로필 드롭다운 상단의 상태 행(점+라벨+저장 버튼, 느낌표 클릭 시 안내 펼침)이 유일한 표시 자리다. 스튜디오 밖 단발 쓰기 트랜잭션(프로필 생성·삭제, 시나리오 생성·복제·가져오기·삭제·칩 순서, 쓰레드 기록·삭제, 전체 복원, 공유 링크 가져오기 — 프로필 전환은 쓰기가 아니라 제외)은 `requestAutoSync()`가 1.2s 디바운스 뒤 자동 업로드한다 — 전체 자동 미러링은 여러 창이 서로를 덮고 실패가 조용해서 제거했고, 자동 트랜잭션 싱크는 충돌(다른 창의 선행 변경) 시 덮지 않고 멈춰 수동 저장으로 유도하며 실패를 토스트로 알린다. 하이드레이션 중에는 홈 프로필 버튼·드롭다운과 SyncButton이 "동기화 중" 펄스(`remoteSync.hydrating`)를, 실패 시 "연결 안 됨"을, 미저장이면 프로필 버튼에 주황 점을 표시한다. 저장 직전 `?index=1`(행 목록·updatedAt)로 다른 창의 선행 변경을 감지해 덮어쓰기 확인을 받는다. 키는 계정을 쪼갠 행 단위다 (분해·조립 규칙은 `lib/accountRows.js`): `account:<id>`(본문 — 버전·쓰레드 제외)·`account:<id>:threads`·`account:<id>:versions:<sid>`(발행 버전 스냅샷)·`accounts-meta`(순서만 — **활성 프로필 id는 기기별 상태라 동기화하지 않는다**, 로컬 저장 전용)·`keywords`. **하이드레이션은 활성 계정 우선 2단계**: 1단계에서 `?keys=` 배치 한 번으로 메타+키워드+활성 계정 본문만 받아 홈을 먼저 채우고, 나머지 계정·쓰레드·버전 행은 백그라운드로 받아 전부 모이면 기준선을 잡고 remoteReady를 켠다(업로드는 그 전까지 차단 — 부분 상태 저장은 고아 정리가 안 받은 행을 지운다). 하이드레이션 사이에 사용자가 손댄 계정은 서버 값으로 덮지 않는다(미저장으로 남는다). 구 형식 둘 — 통짜 `accounts` 블롭(Vercel 본문 한도 4.5MB에 닿아 큰 저장이 조용히 413 거부됐던 형식)과 통짜 계정 행(버전·쓰레드 인라인, `threads` 키 포함 여부로 식별) — 은 하이드레이션이 최초 1회 분리 행으로 마이그레이션(부속 행 먼저 → 본문 행 마지막이 완료 표식, 중간에 끊겨도 재시도)한다. 저장은 참조 비교로 바뀐 행만 전송(쓰레드 기록이 본문을, 발행이 쓰레드를 다시 실어 나르지 않게 `subBaselineRef`가 부속 행 기준선을 따로 든다), 삭제는 `data: null` PUT. **계정 객체 참조가 곧 미저장 신호**다: `patchActive`·`updateScenario`·`useStageItems.write`가 무변경 업데이터(같은 참조 반환)를 스킵하는 사슬을 유지할 것 — 끊기면 빌더만 열어도 미저장 배지가 켜진다. 발행 버전 스냅샷은 시나리오 전체 사본이라 페이로드 주범 — `VERSION_LIMIT`(5)을 늘리지 말 것. **주의: 행 분리 이후 구 클라이언트(커밋된 docs/ = GitHub Pages 레거시 빌드)로 "서버에 저장"을 누르면 본문만 있는 행을 통짜로 재저장해 서버의 쓰레드·버전이 유실된다** — 레거시 배포에서는 편집·저장하지 말 것.
